...
vi.mock("../api/httpClient", async () => {
    const actual = await vi.importActual("../api/httpClient");
    return {
        ...actual,
        api: {
            uploadFile: vi.fn(),
            createPlan: vi.fn(),
            executePlan: vi.fn(),
            downloadUrl: vi.fn((id) => `/api/outputs/${id}`),
            getAudit: vi.fn(),
        },
    };
});
...
describe("App workflow", () => {
    it("renders upload, plan and result panels in sequence", async () => {
        ...
        expect(mockedApi.executePlan).toHaveBeenCalledWith(["f-1"], expect.objectContaining({ id: "plan-1" }), "confirm:plan-1");
    });
    it("surfaces user friendly error when upload fails", async () => {
        const { ApiError } = await import("../api/httpClient");
        mockedApi.uploadFile.mockRejectedValueOnce(new ApiError({
            status: 400,
