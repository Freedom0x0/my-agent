...
class DemoPlanner:
    ...
    def plan(
        ...
        text = request or ""
        primary_sheet, primary_file_id = _pick_primary_sheet(inspections, sheet_catalog)
        ...
        if "对比" in text or "比较" in text:
            if len(sheet_catalog) < 2:
                raise AmbiguousRequestError("对比任务需要至少两个工作表")
            ref1 = sheet_catalog[0].ref
            ref2 = sheet_catalog[1].ref
            primary_sheet_for_compare = inspections
            insp_by_name: dict[str, SheetInspection] = {}
            for insp in primary_sheet_for_compare:
                for sh in insp.sheets:
                    insp_by_name[sh.name] = sh
            s1 = insp_by_name.get(sheet_catalog[0].sheet_name)
            s2 = insp_by_name.get(sheet_catalog[1].sheet_name)
            if s1 is None or s2 is None:
                raise AmbiguousRequestError("对比任务需要有效的工作表")
            try:
